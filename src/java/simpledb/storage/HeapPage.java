...
public class HeapPage implements Page {
    ...
    public void deleteTuple(Tuple t) throws DbException {
        ...
        if (!Objects.equals(t.getRecordId().getPageId(),this.pid)){
            throw new DbException("is  already empty");
        }

        int num = t.getRecordId().getTupleNumber();
        if (checkBit(header[num / 8], num % 8)) {
            setBit(num, 0);
    ...
    public int getNumEmptySlots() {
        // some code goes here
        return this.numSlots - this.getNumNotEmptySlots();
    }


    public int getNumNotEmptySlots() {
        int res = 0;
        for (int i = 0; i < numSlots; i++) {
            if (checkBit(i)) {
                res++;
    ...
    public boolean isSlotUsed(int i) {
        // some code goes here
        return (this.header[i / 8] & (1L << i % 8)) != 0;
    }
    ...
    /**
     * @return 返回第一个为0的位，没有返回-1
     */
    private int checkBit(byte b) {
        //todo
        return -1;
    }

    private boolean checkBit(int index) {
        return (header[index / 8] & 1L << index % 8) != 0;
    }


    private boolean checkBit(byte b, int index) {
        //todo
        return (b & 1L << index) != 0;
    }
    ...
    private void setBit(int index, int mask) {
        byte b = header[index / 8];
        int m = 1 << (index % 8);
        if (mask == 1) {
            header[index / 8] = (byte) (b | m);
        } else {
            header[index / 8] = (byte) (b & ~m);
        }
