...
public class SeqScan implements OpIterator {
    ...
    public TupleDesc getTupleDesc() {
        // some code goes here
        TupleDesc originDesc = Database.getCatalog().getTupleDesc(this.tableId);
        if (this.tableAlias==null){
            return originDesc;
        }
        String[] fieldNames = new String[originDesc.numFields()];
        Type[] types = new Type[originDesc.numFields()];
        for (int i = 0; i < fieldNames.length; i++) {
            types[i] = originDesc.getFieldType(i);
            fieldNames[i] = this.tableAlias+"."+ originDesc.getFieldName(i);
        }
